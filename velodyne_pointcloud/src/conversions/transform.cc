  ...
  , lastPossibleMsgTime_(ros::Time(0))
{
  ...
  if ((input_rosbag_path_ != ""))
  {
    ...
    std::filesystem::permissions(output_rosbag_path_,
  ...
  // Container
  container_ptr = boost::shared_ptr<PointcloudXYZIRT>(new PointcloudXYZIRT(config_.max_range, config_.min_range,
  ...
  output_ = node.advertise<sensor_msgs::PointCloud2>("/anymal/velodyne/points_undistorted", 1);

  outputDistorted_ = node.advertise<sensor_msgs::PointCloud2>("/anymal/velodyne/points", 1);
 ...
 */
void Transform::processScan(const any_velodyne_msgs::VelodyneScan::ConstPtr& scanMsg)
{
  counter_++;
