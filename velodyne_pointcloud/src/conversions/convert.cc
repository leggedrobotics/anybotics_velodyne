...
Convert::Convert(ros::NodeHandle node, ros::NodeHandle private_nh, std::string const& node_name)
  : data_(new velodyne_rawdata::RawData()), first_rcfg_call(false), diagnostics_(node, private_nh, node_name)
{
  ...
  {
    container_ptr_ = boost::shared_ptr<PointcloudXYZIRT>(new PointcloudXYZIRT(
        config_.max_range, config_.min_range, config_.target_frame, config_.fixed_frame, data_->scansPerPacket()));
  }

  output_ = node.advertise<sensor_msgs::PointCloud2>("/lidar/points_undistorted", 10);
  ...
  velodyne_scan_ = node.subscribe("/lidar/packets", 10, &Convert::processScan, (Convert*)this,
