...
OrganizedCloudXYZIRT::OrganizedCloudXYZIRT(const double max_range, const double min_range,
  ...
  : DataContainerBase(max_range, min_range, target_frame, fixed_frame, num_lasers, 0, false, scans_per_block, tf_ptr, 6,
                      "x", 1, sensor_msgs::PointField::FLOAT32, "y", 1, sensor_msgs::PointField::FLOAT32, "z", 1,
                                    ...
                                    const float distance, const float intensity, const float time)
{
   ...
   * which are not in range to NaN, to maintain the organized structure. Neighbouring scan lines have to result in
   * neighbouring points in the organized cloud. The feature is a great one, it is simple but enables a fast estimation
   * of good point normals and also of surface reconstruction. This helps e.g. for robustness of registering point
   * clouds and therefore for a more robust localization.
   */
  /**
   * In the organized point cloud, horizontal neighbouring points (aligned in row) are the lidar points with close laser
   * id and the same azimuth. Vertical neighbouring points (aligned in column) are the lidar points with close azimuth
   * and the same laser id.
   */
