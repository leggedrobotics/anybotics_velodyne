...
int RawData::setupOffline(std::string calibration_file, double max_range_, double min_range_)
{
  ...
  ROS_DEBUG_STREAM("Data ranges to publish: [" << config_.min_range << ", " << config_.max_range << "]");
 ...
 */
void RawData::unpack(const any_velodyne_msgs::VelodynePacket& pkt, DataContainerBase& data,
 ...
 *  @param pc shared pointer to point cloud (points are appended)
 */
void RawData::unpack_vlp16(const any_velodyne_msgs::VelodynePacket& pkt, DataContainerBase& data,
                           const ros::Time& scan_start_time)
{
  float azimuth;
  float azimuth_diff;
  ...
  /**
   * The information from two Firing Sequences of 16 lasers is contained in one Data Block. Each Data Block contains
   * 32(2*16) data points. Each packet contains the data from 24 firing sequences. Only one Azimuth is returned per Data
   * Block.
   */
  ...
  for (int block = 0; block < BLOCKS_PER_PACKET; block++)
  {
    ...
    if (block < (BLOCKS_PER_PACKET - 1))
    {
      ...
      if (raw_azimuth_diff < 0)  // raw->blocks[block+1].rotation - raw->blocks[block].rotation < 0)
      {
        ...
        ROS_DEBUG_STREAM_THROTTLE(60, "Packet containing angle overflow, first angle: "
